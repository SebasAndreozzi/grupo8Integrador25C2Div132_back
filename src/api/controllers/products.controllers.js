...
export const createProduct = async (req, res) => {
    ...
    try {
        ...
        let { nombre, precio, tipo, img } = req.body;
        console.log(req.body);
        console.log(`Nombre producto: ${nombre}`);


        // Optimizacion 1: Validamos de datos de entrada
        if(!tipo || !precio || !nombre || !img) {
            return res.status(400).json({
                message: "Datos invalidos, asegurate de enviar todos los campos"
            });
        }

        let [rows] = await ProductModel.insertProduct(nombre, precio, tipo, img);
...
export const modifyProduct = async (req, res) => {
    try {
        /*{
            ...
            "precio": "https://burgernj.com/wp-content/uploads/2021/05/Salmon-Burger_.jpg",
            "tipo": "food",
            "img": "2500.00",
            "activo": "1"
        }*/
        ...
        let { id, nombre, precio, tipo, img, activo } = req.body;


        // Optimizacion 1: Validacion basica de todos los campos recibidos en el body
        if(!id || !nombre || !precio || !tipo || !img || !activo) {
            return res.status(400).json({
                message: "Faltan campos requeridos"
            });
        }

        let [result] = await ProductModel.updateProduct(nombre, precio, tipo, img, activo, id);
